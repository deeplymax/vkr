...
function Queue() {
  ...
  let configurationFiles = [
    "./judge-queue-configuration.json"
  ];
  this.configuration =
...
Queue.prototype = {
  ...
  _resolveConfiguration: function(configuration) {
    ...
    configuration["backServer"] =
      process.env["JUDGE_QUEUE_BACK_SERVER"] ||
        configuration["backServer"] || "http://judge-back:7070";
    ...
    databaseConfigurations["service"] =
      this._extractEnvironmentConfiguration("JUDGE_QUEUE_DISCOVERY_DATABASE") ||
        databaseConfigurations["service"];
    databaseConfigurations["queue"] =
      this._extractEnvironmentConfiguration("JUDGE_QUEUE_QUEUE_DATABASE") ||
        databaseConfigurations["queue"] || {
          "host": "judge-queue-db",
          "port": "6379"
        };
    databaseConfigurations["task"] =
      this._extractEnvironmentConfiguration("JUDGE_QUEUE_TASK_DATABASE") ||
        databaseConfigurations["task"] || {
          "url": "mongodb://judge-task-db:27017/judge",
          "options": { }
        };
